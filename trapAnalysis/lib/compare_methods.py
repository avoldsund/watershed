...
def compare_two_dictionaries_where_values_are_tuples_with_two_arrays(d1, d2):
    """
    Dictionary with number as key, values as tuple with two arrays.
    Returns True if the two dictionaries are equal, False if not.
    :param d1: Dictionary 1.
    :param d2: Dictionary 2.
    :return: True if d1 and d2 are equal, False if not.
    """
    ...
    if len(d1) != len(d2):  # Test if the number of keys are not equal
        ...
        if set(d1) != set(d2):  # Test if the keys are not equal
            ...
            for key in d1:  # Test if all values in d1 and d2 are equal
                values_are_equal = (np.array_equal(d1[key][0], d2[key][0]) and np.array_equal(d1[key][1], d2[key][1]))
                if not values_are_equal:
                    return False
